...
public class hellow {
	...
	public hellow() {
		for (int i = 0; i < 200; i++) {
			...
			if (i < 100 && i % 2 == 0) {
				...
				e1.setData(Calendar.getInstance(Locale.getDefault()).getTimeInMillis()
						);
			} else if (i >= 100 && i % 2 == 0) {
	...
	public @ResponseBody
	List<UserObject> getAllUsers(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) {
	...
	List<Evento> getEventi(HttpServletRequest request,
		...
		Evento evnt = new Evento();
		List<Evento> evappl = new ArrayList<Evento>();
		data = data - data % 86400000;
		
		for (i = 0; i < yep.size(); i++) {

			evnt = yep.get(i);
			long dataev = evnt.getData();
			dataev = dataev - dataev % 86400000;
			
			if (dataev == data && evnt.getAmbito().equals(user_ambito)
					&& evnt.getRuolo() <= user_ruolo) {
				evappl.add(evnt);
			}
	...
	List<Evento> getEventi(HttpServletRequest request,
		...
		for (k = 0; k < yep.size(); k++) {
			...
			if (evnt.getAmbito().equals(user_ambito)
					&& evnt.getRuolo() <= user_ruolo) {
				evappl.add(evnt);
			}
